...
ggplot(data= Plot_ds2, aes(x=Year, y= value, group = variable, colour = variable)) + geom_line(size = 1) + ggtitle("Annual Percent Change") +
	   theme(plot.title = element_text(size = 15, lineheight=.8, face="bold"))+ coord_cartesian(ylim = c(0, 10)) + xlab("Years") + ylab("Average Temperature") +
	   theme(axis.title = element_text(size = 13), axis.text = element_text(size = 13)) +
	   theme(plot.title = element_text(size = 15, lineheight=.8, face="bold")) + facet_grid(Plot_ds2$variable ~., scales = "free_y") + theme(legend.position="none")
