...
Based on the Annual Percent Change Graph bellow, we can see that the average temperature of the Ocean has higher amplitude of changes over years than land temperature. Oceans are heating and cooling much faster than land due to water`s lower density. There are few key factors that influence land temperature changes, besides the Gulf stream.WWith Rapid deforestation, we reduce the enviroment's capacity to absorb the greenhouse gases that fuel global warming. Fewer forests allow elevated level of these gases to enter the atmosphere (~15% carbon) adn trap solar rays, thereon heating up the surface that once been protected by the forest. 
